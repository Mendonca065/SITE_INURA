...
export default function LightCurtain(props: LightCurtainProps) {
    const {
        style,
        background = "#000000",       // Preto puro INURA
        baseColor = "#7C2D12",        // Laranja queimado profundo
        accentColor = "#F97316",      // Laranja primário INURA
        highlight = "#FFEDD5",        // Centro incandescent warm-white
        density = 130,
        speed = 40,
        curtain,
        hover = 100,
    ...
    vRef.current = {
        ...
        accent: accentColor,
        high: highlight,
        density: clampN(num(density, 130), 10, 200) / 50,
        speed: clampN(num(speed, 40), 0, 100) / 50,
        cw: clampN(num(cur.curtainWidth, 40), 10, 250) / 100,
        spread: clampN(num(cur.spread, 30), 0, 200) / 100,
        striation: clampN(num(cur.striation, 80), 0, 100) / 100,
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            const bg = parseColor(v.bg, [0, 0, 0])
            const base = parseColor(v.base, [0.486, 0.176, 0.07])
            const accent = parseColor(v.accent, [0.976, 0.45, 0.086])
            const high = parseColor(v.high, [1.0, 0.93, 0.835])
