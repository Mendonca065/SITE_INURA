...
const ARC_DEFAULTS: Required<ArcGroup> = { center: 50, drop: 0, thickness: 44, falloff: 600 }

export interface DataPixelArcProps {
...
export default function DataPixelArc(props: DataPixelArcProps) {
  const {
    style,
    background = "#000000",       // Fundo preto puro INURA
    baseColor = "#7C2D12",        // Tom profundo queimado/âmbar
    accentColor = "#F97316",      // Laranja primário INURA
    highlight = "#FFEDD5",        // Brilho quente incandescente
    density = 161,
    dotSize = 100,
    speed = 100,
    pointerStrength = 34,
  ...
  vRef.current = {
    ...
    accent: accentColor,
    high: highlight,
    density: Math.round(clampN(num(density, 161), 24, 260)),
    dotSize: clampN(num(dotSize, 100), 20, 100) / 100,
    speed: clampN(num(speed, 100), 0, 100) / 50,
    pointerStrength: clampN(num(pointerStrength, 34), 0, 100) / 100,
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const cb = parseColor(v.base as string, [0.486, 0.176, 0.07])
      const ca = parseColor(v.accent as string, [0.976, 0.45, 0.086])
      const chh = parseColor(v.high as string, [1.0, 0.93, 0.835])
